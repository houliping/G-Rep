...
from mmdet.ops.gmm import GMM, bbox2gaussian

class MaxKLDAssigner(BaseAssigner):
    def __init__(self,
                 pos_iou_thr,
                 neg_iou_thr,
                 min_pos_iou=.0,
    ...
    def assign(self, points, gt_rbboxes, overlaps, gt_rbboxes_ignore=None, gt_labels=None, ):
        ...
        kld = self.kld_mixture2single(gmm, bbox2gaussian(gt_rbboxes)) #(K,N)
        overlaps = torch.exp(1 / (2 + kld.sqrt())) - 1
        # overlaps= 1 / (2+kld.sqrt())
        # print('kld')
        # print(kld)
        # print('overlaps')
        # print(overlaps)
        ...
        if (self.ignore_iof_thr > 0 and gt_rbboxes_ignore is not None
            ...
            if self.ignore_wrt_candidates:
                ignore_overlaps = 1 / (2 + self.kld_mixture2single(gmm, bbox2gaussian(gt_rbboxes_ignore)))
