...
class GRepReassignHead(nn.Module):
    def __init__(self,
                 num_classes,
                 in_channels,
                 feat_channels=256,
        ...
        super(GRepReassignHead, self).__init__()
    ...
    def loss_single(self, cls_score, pts_pred_init, pts_pred_refine, labels,
        ...
        if self.use_reassign:
            # use_cfa  calculate the loss_pts_init is same as  not use_cfa
            ...
            # print('pts_pred_init_norm', pts_pred_init_norm)
            rbbox_gt_init_norm = rbbox_gt_init[pos_ind_init]
            # print('rbbox_gt_init_norm', rbbox_gt_init_norm)
    ...
    def loss(self,
        ...
        ## use_cfa  cfa is a key import point
        if self.use_reassign:  # get num_proposal_each_lvl and lvl_num
            ...
            # print('num_proposals_each_level', num_proposals_each_level)
            # [17424, 4356, 1089, 289, 81]
            # [14400, 3600, 900, 225, 64]
        ...
        if cfg.init.assigner['type'] == 'ConvexAssigner':
            ...
            raise NotImplementedError # point based on boxes is also a research point
        ...
        if self.use_reassign:
            cls_reg_targets_refine = self.reassign_point_target(
                points_list,
                valid_flag_list,
                gt_rbboxes,
                img_metas,
            ...
            # levels_to_images 此过程与images_to_levels相反 目的是保持与 labels_list, label_weights_list, rbbox_gt_list_refine,
            # convex_weights_list_refine 的levels统一
            ...
            with torch.no_grad():
                ...
                pos_losses_list, = multi_apply(self.get_pos_loss, cls_scores,
                                               pts_coordinate_preds_refine, labels_list,
            ...
            if num_pos:
                ...
                # print('pos_pts_pred_refine', pos_pts_pred_refine.shape)
                pos_rbbox_gt_refine = rbbox_gt_refine[pos_inds_flatten]
                # print('pos_rbbox_gt_refine', pos_rbbox_gt_refine.shape)
                pos_convex_weights_refine = convex_weights_refine[pos_inds_flatten]

                # print('pos_normalize_term.reshape(-1, 1)', pos_normalize_term.reshape(-1, 1).shape, pos_normalize_term.reshape(-1, 1))
    ...
    def get_pos_loss(self, cls_score, pts_pred, label, rbbox_gt,
        ...
        # 此处的pts_pred_init为第一阶段的预测 相当于anchor

    def reassign(self, pos_losses, label, label_weight, pts_pred_init, convex_weight, gt_rbbox,
                     pos_inds, pos_gt_inds, num_proposals_each_level=None, num_level=None):
        '''
              Fit loss to Gaussian distribution and separate positive, ignore, negative samples again
              Args:
                  pos_losses: Losses of all positive samples in
                              single image.
                  label:classification target of each anchor with
                      shape (num_anchors,).
                  label_weight:label_weight (Tensor): Classification loss weight of each
                      anchor with shape (num_anchors).
                  pts_pred_init:  the init stage predicted pts
                  convex_weight:  convex weight of each anchor with shape
                  gt_rbbox: gt rbbox
                  pos_inds:  Index of all positive samples got from
                      first assign process.
                  pos_gt_inds:  Gt_index of all positive samples got
                      from first assign process. pos_inds_list_refine  the refine stage after max iou assign
                  num_proposals_each_level: 每个stage下面有多少 proposals
                  num_level: 5
        ...
        num_proposals_each_level_.insert(0, 0)  # 在0的位置插一个0
        inds_level_interval = np.cumsum(num_proposals_each_level_)  # 对proposal进行求和
        pos_level_mask = []
    ...
    def reassign_point_target(self,
                         proposals_list,
                         valid_flag_list,
                         gt_rbboxes_list,
                         img_metas,
        ...
        (all_labels, all_label_weights, all_rbbox_gt, all_proposals,
         all_proposal_weights, pos_inds_list, neg_inds_list, all_gt_inds) = multi_apply(
            self.reassign_point_target_single,
            proposals_list,
        ...
        #cfa_point_target_single is same as point_target_single
        pos_inds = []
        pos_gt_index = []
        ...
        # print('pos_gt_index', pos_gt_index)
        # 对pos_inds pos_gt_index进行统计
        # 同时返回 all_labels  all_label_weights all_rbbox_gt all_proposals
        #         all_proposal_weights 与point_target函数相比 此处并没有进行images_to_levels
        return (all_labels, all_label_weights, all_rbbox_gt, all_proposals,
                all_proposal_weights, pos_inds, pos_gt_index)


    def reassign_point_target_single(self,
                                flat_proposals,
                                num_level_proposals,
                                valid_flags,
                                gt_rbboxes,
        ...
        if sampling:
            ...
            if cfg.assigner.type != "ATSSAssigner" and cfg.assigner.type != "ATSSConvexAssigner" \
                    ...
                    and cfg.assigner.type != "ATSSBcdAssigner":
                assign_result = bbox_assigner.assign(proposals, gt_rbboxes, overlaps,
                                                     gt_rbboxes_ignore, gt_labels)
            else:
    ...
    def point_target_refine(self,
        ...
        (all_labels, all_label_weights, all_rbbox_gt, all_proposals,
         all_proposal_weights, pos_inds_list, neg_inds_list, all_gt_inds_list) = multi_apply(
            self.point_target_single_refine,
            proposals_list,
            num_level_proposals_list,
            valid_flag_list,
            gt_rbboxes_list,
    ...
    def point_target_single_refine(self,
                                   flat_proposals,
                                   num_level_proposals,
                                   valid_flags,
                                   gt_rbboxes,
    ...
    def get_bboxes_single(self,
        ...
        for i_lvl, (cls_score, points_pred, points) in enumerate(
            ...
            bbox_pred = self.points2gmm2rotrect(points_pred)
