...
if __name__ == '__main__':
    ...
    setup(
        ...
        ext_modules=[
            ...
            # minareabbox_cuda
            make_cuda_ext(
                name='minareabbox_cuda',
                module='mmdet.ops.minareabbox',
                sources=['src/minareabbox_cuda.cpp', 'src/minareabbox_kernel.cu']),
            make_cuda_ext(
                name='nms_cpu',
            ...
            make_cuda_ext(
                name='dis_pointLine',
                module='mmdet.ops.dis_pointLine',
                sources=[
                    'dis_pointLine.cpp',
                    'dis_pointLine_kernel.cu',
                ]),
            make_cuda_ext(
                name='rotated_giou_cuda',
                module='mmdet.ops.iou',
                sources=[
                    'src/rotated_giou_cuda.cpp',
                    'src/rotated_giou_kernel.cu',
                ]),
            make_cuda_ext(
                name='rotated_iou_cuda',
                module='mmdet.ops.iou',
                sources=[
                    'src/rotated_iou_cuda.cpp',
                    'src/rotated_iou_kernel.cu',
                ]),
            CUDAExtension(name='mmdet.ops.torch_batch_svd._c',
                          sources=['mmdet/ops/torch_batch_svd/csrc/bindings.cpp',
                                   'mmdet/ops/torch_batch_svd/csrc/torch_batch_svd.cu'],
                          libraries=["cusolver", "cublas"],
                          extra_compile_args={
                              'cxx':
                                  ['-O2', '-Immdet/ops/torch_batch_svd/include'],
                              'nvcc':
                                  ['-O2', '-Immdet/ops/torch_batch_svd/include']
                          })
            # ,
            # make_cuda_ext(
            #     name='points_kmeans_cuda',
            #     module='mmdet.ops.kmeans',
            #     sources=[
            #         'src/points_kmeans_cuda.cpp',
            #         'src/points_kmeans_kernel.cu',
            #     ])
